#!/usr/bin/env node
// os-p3.test.mjs — OS-P3 eval (K1): evals.json carries all 31 method rows with
...
// ── evals.json: 31 rows, self-consistent, machine-written ──
...
ok(ev.results.length === 31, `31 method rows (got ${ev.results.length})`);
const names = new Set(ev.results.map(r => r.method));
ok(names.size === 31, "31 distinct methods");
for (const m of ["unit_root", "soch_profile", "channel_attribution", "namh_hurst", "ksg_te", "sri_daily", "namh_pipeline", "namh_reproduce", "soch_robustness"])
...
ok((table.match(/<tr><td/g) || []).length === 31, "table renders all 31 rows");
